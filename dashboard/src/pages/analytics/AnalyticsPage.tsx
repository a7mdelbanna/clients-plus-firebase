...
export default function AnalyticsPage() {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      setLoading(true);
      const analyticsData = await analyticsService.getSalesAnalytics(user.companyId, {
        ...
        <TabPanel value={tabValue} index={3}>
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              <Paper sx={{ p: 3 }}>
                <Typography variant="h6" gutterBottom>
                  {isRTL ? 'توزيع طرق الدفع' : 'Payment Methods Distribution'}
                </Typography>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={Object.entries(analytics.paymentMethodBreakdown).map(([method, data]) => ({
                        name: method,
                        value: data.amount,
                    ...
                    >
                      {Object.keys(analytics.paymentMethodBreakdown).map((_, index) => (
            ...
            <Grid item xs={12} md={6}>
              <Paper sx={{ p: 3 }}>
                <Typography variant="h6" gutterBottom>
                  {isRTL ? 'تفاصيل طرق الدفع' : 'Payment Methods Details'}
                </Typography>
                <Box sx={{ mt: 2 }}>
                  {Object.entries(analytics.paymentMethodBreakdown).map(([method, data], index) => (
                    <Box key={method} sx={{ mb: 3 }}>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {data.transactionCount} {isRTL ? 'معاملة' : 'transactions'}
                      </Typography>
                    </Box>
                  ))}
                </Box>
