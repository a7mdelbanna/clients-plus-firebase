...
import { categoryService, type ClientCategory } from '../../../services/category.service';
...
const ClientCategoriesPage: React.FC = () => {
  ...
  const [filteredCategories, setFilteredCategories] = useState<ClientCategory[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        unsubscribe = categoryService.subscribeToCategories<ClientCategory>(
          cId,
          'client',
          (updatedCategories) => {
  ...
  const handleMenuOpen = (event: React.MouseEvent<HTMLElement>, category: ClientCategory) => {
  ...
  const handleDelete = async () => {
    ...
    if (selectedCategory.itemCount && selectedCategory.itemCount > 0) {
      toast.error('لا يمكن حذف فئة مرتبطة بعملاء');
      handleMenuClose();
      return;
    }
    ...
    if (window.confirm(`هل أنت متأكد من حذف الفئة "${selectedCategory.name}"؟`)) {
      try {
        await categoryService.deleteCategory(selectedCategory.id!, 'client');
          ...
          <Paper sx={{ p: 8, textAlign: 'center', borderRadius: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 4 }}>
              {searchTerm 
                ...
                : "لم تقم بإنشاء أي فئات بعد. أنشئ واحدة لتنظيم عملائك."}
            </Typography>
            {!searchTerm && (
              <Button
                variant="contained"
                ...
                <Card sx={{ borderRadius: 2, '&:hover': { boxShadow: theme.shadows[4] } }}>
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                      <Chip
                        label={`${category.itemCount || 0} عميل`}
                        size="small"
                        variant="outlined"
                      />
                      <Chip
